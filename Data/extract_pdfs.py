...
# Input  : aptnotes_pdfs/*.pdf   (change with --in)
# Output : Data/extracted_pdfs/<slug>-<sha1_8>/{p001.txt, full.txt, meta.json}
# CSV    : Data/extracted_pdfs/extracted_iocs.csv (file,page,kind,value)
# ---------- Dependencies ----------
try:
    import fitz  # PyMuPDF
except ImportError:
...
ATTACK_RX = re.compile(r'\bT\d{4}(?:\.\d{3})?\b')
...
def extract_iocs_from_text(text: str):
    """
    Return list of (kind, value) with de-duplication.
    Captures: url, domain (from url + bare domains), ipv4, md5, sha1, sha256, email, attack_id
    """
    out = []
...
def write_iocs(all_iocs, out_csv: Path):
    ...
    with out_csv.open("w", encoding="utf-8", newline="") as f:
        ...
        for r in all_iocs:
            key = (r["file"], r["page"], r["kind"], r["value"].lower())
            if key in seen:
                continue
            seen.add(key)
            val = r["value"].replace('"', '""')
            f.write(f"{r['file']},{r['page']},{r['kind']},\"{val}\"\n")


def main():
    ap = argparse.ArgumentParser(description="Extract text/metadata/IOCs from PDFs.")
    ap.add_argument("--in",  dest="in_dir",  default="aptnotes_pdfs", help="Input folder")
    ap.add_argument("--out", dest="out_dir", default="Data/extracted_pdfs", help="Output folder")
    ...
    if all_iocs and not args.no_iocs:
        write_iocs(all_iocs, out_dir / "extracted_iocs.csv")
