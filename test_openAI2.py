...
def load_csv_data():
    """Load TTP input and matched groups CSVs."""
    ...
    # Sort by descending score if available
...
def analyze_TTP(input_ttps, matched_df):
    """Send top group data + TTPs to GPT for CTI-style report generation."""
    mapping_summary = "\n".join([
...
def parse_ai_response(text):
    """Extract main report sections from GPT response."""
...
def generate_word_report(report_text, input_ttps):
    """Convert GPT analysis text into a structured and styled Word report."""
    parsed = parse_ai_response(report_text)

    output_dir = Path("Generated_Reports")
    output_dir.mkdir(parents=True, exist_ok=True)
    filename = f"Threat_Report_{datetime.now():%Y%m%d_%H%M%S}.docx"
    ...
    doc.add_paragraph("ICT3214 – Security Analytics | AY2025/2026 Trimester 1")
    ...
    footer_para.text = "Confidential – Security Analytics Assignment 2025"
